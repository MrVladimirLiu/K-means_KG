...
class GloVe():
    ...
    def __init__(self, file_path):              # structure word embedding
        self.dimension = None
        self.embedding = dict()
        with open(file_path, 'r') as f:
            for line in f.readlines():
                strs = line.strip().split()     # split data
                word = strs[0]
    ...
    def _fix_word(self, word):
        ...
        cnt = 0
        for term in terms:
            v = self.embedding.get(term)       # value
            if v is None:
                subterms = term.split('-')
            ...
            if v is not None:
                ret += v
                cnt += 1
        return ret / cnt >0 if cnt > 0 else None
